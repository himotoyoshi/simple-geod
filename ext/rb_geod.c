...
static VALUE
rb_geodesic_measure_polygon (VALUE self, VALUE vlats, VALUE vlons, VALUE vn)
{
  Geodesic *geodesic;
  int n;
  double *lats, *lons;
  ...
  n = NUM2INT(vn);
  geod_polygonarea(geodesic->ref, lats, lons, n, &area, &perimeter);
  return rb_ary_new3(2, rb_float_new(area), rb_float_new(perimeter));
}
...
Init_simple_geod ()
{
  ...
  rb_define_private_method(rb_cGeodesic, "_measure_polygon", rb_geodesic_measure_polygon, 3);
}
